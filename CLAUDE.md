...
- **Completed so far:**
  ...
  - **Recent-track exclusion (DONE, 2026-07-19, owner-requested):** re-attempting the **same emotion × genre** path kept returning the same few songs when the pool was narrow (Stage-1's random window is a no-op once the whole matching set fits inside the ~1000-row window — K-Pop × sad is only 17 tracks, smaller than one playlist; two draws of `size` from `N` repeat ~`size²/N`). Fix: the frontend remembers the `track_id`s served for each `(emotion × genre filter)` context this session (`frontend/js/recent_tracks.js`, `sessionStorage.recent_tracks`, rolling cap 300/context) and passes them to `generate_playlist` as a new `exclude_ids` arg; `loading.js` reads before generating + records on success. Recommender's `_sample_excluding` splits the fetched window into fresh/stale, draws from fresh, and **backfills from stale when unseen tracks run out** — so exclusion never shortens a playlist or adds a `playlist_failed` path. Applied in Python over the already-fetched window ⇒ **no SQL/index/migration change**, exact for the narrow pools that repeat, near-no-op for huge ones; recommender stays a pure function (seeded tests unaffected; `exclude_ids=None` = original path bit-for-bit). Bridge sanitises `exclude_ids` like `genres`. **No re-roll button** (owner declined) — works on the existing redo-mood path. Session-scoped only, so not the out-of-scope long-term personalisation. Docs: RECOMMENDATION.md + FRONTEND.md § "Recent-track exclusion". Unit suite at 200. Not yet exercised in the real app.
